...
export async function GET(req) {
    try {
        ...
        let doc = await EssentialService.findOne({ status: mode });
        
        if (!doc && mode === 'draft') {
            doc = await EssentialService.findOne({ status: 'live' });
        }
...
export async function PUT(req) {
    try {
        ...
        let draft = await EssentialService.findOne({ status: 'draft' });
        const live = await EssentialService.findOne({ status: 'live' });
        ...
        const updatedDraft = await EssentialService.findOneAndUpdate(
            { status: 'draft' },
            {
                $set: {
