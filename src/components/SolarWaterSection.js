...
export default function SolarWaterSection() {
    ...
    const [sectionTitle, setSectionTitle] = useState('Solar & Water Solutions');
    const [showTitleModal, setShowTitleModal] = useState(false);
    const [titleFormText, setTitleFormText] = useState('Solar & Water Solutions');
    ...
    useEffect(() => {
        ...
        window.addEventListener('admin_solarwater_updated', loadServices);
        ...
        return () => {
          ...
          window.removeEventListener('admin_solarwater_updated', loadServices);
                ...
                <div style={{ position: 'relative' }}>
                    ...
                    {servicesList.map((service, index) => (
                        ...
                        >
                            ...
                            {service.price && (
                                <p style={{ color: '#10b981', fontWeight: '700', marginTop: '4px', fontSize: '14px' }}>
              ...
              <div className="hero-modal-overlay" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999999 }} onClick={() => setShowModal(false)}>
                <div className="hero-modal-content" onClick={(e) => e.stopPropagation()} style={{ background: '#ffffff', borderRadius: '16px', maxWidth: '460px', width: '90%', maxHeight: '85vh', overflowY: 'auto', padding: '24px', position: 'relative', zIndex: 1000000 }}>
                  ...
                  <h3 style={{ fontSize: '20px', fontWeight: '800', color: '#0f172a', marginBottom: '20px' }}>
                    {editingItem ? "Edit Solution Card" : "Add New Solution Card"}
                  </h3>
                  ...
                  <form onSubmit={handleSaveCard}>
                    <div style={{ marginBottom: '14px' }}>
                      ...
                      <input
                        type="text"
                        required
                        placeholder="e.g. Solar Panels, Solar Water Heaters"
                      ...
                      <button
                        ...
                        onClick={() => {
                          sessionStorage.setItem('linking_card_data', JSON.stringify({
                            sectionType: 'solarwater',
                            id: editingItem ? editingItem.id : `solar-${Date.now()}`,
                    ...
                    >
                      {editingItem ? "Save Card Changes" : "Publish Card Live"}
                    </button>
                  </form>
                </div>
              </div>
